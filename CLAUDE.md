...
Deployed to **base-sepolia testnet** (worker `veristat` on workers.dev, D1 `veristat` remote id wired in `wrangler.jsonc`, facilitator `https://x402.org/facilitator`). `PAY_TO_ADDRESS` is still the zero-address placeholder. Mainnet cutover is a gated checklist in `docs/ROADMAP.md` Phase 3 — do not flip `NETWORK` to `eip155:8453` ad hoc.
